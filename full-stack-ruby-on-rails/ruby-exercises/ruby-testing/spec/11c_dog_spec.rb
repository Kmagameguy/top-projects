...
describe Dog do
  ...
  subject(:pepper) { Dog.new(name: 'Pepper', breed: 'Dachshund', color: 'Speckled') }
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog has method nameshared with other classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(pepper).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(pepper).to be_sleeping
  end
end
