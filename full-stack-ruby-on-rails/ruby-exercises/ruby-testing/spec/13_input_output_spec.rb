...
describe NumberGame do
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_end) { described_class.new(5, '5', 4) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        tough_phrase = "That was hard. It took you #{game_end.count} guesses!\n"
        expect { game_end.final_message }.to output(tough_phrase).to_stdout
      end
    end
  end
end
