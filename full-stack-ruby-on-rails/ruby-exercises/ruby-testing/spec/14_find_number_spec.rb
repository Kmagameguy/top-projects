...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:number_seven) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        guess = number_seven.make_guess
        expect(guess).to eq 7
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:number_eight) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        guess = number_eight.make_guess
        expect(guess).to eq 8
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:number_one) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        guess = number_one.make_guess
        expect(guess).to eq 1
      end
    end

    context 'when min and max both equal 3' do
      subject(:number_three) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        guess = number_three.make_guess
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:correct_number) { double('random_number', value: 42) }
      subject(:game_end) { described_class.new(1, 60, correct_number, 42) }
    ...
    context 'when guess and random_number are not equal' do
      let(:incorrect_number) { double('random_number', value: 13) }
      subject(:game_continue) { described_class.new(1, 25, incorrect_number, 20) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:weird_game) { described_class.new(5, 8, number_range, 7) }

      before do
        weird_game.update_range
      end

      it 'updates min to the same value as max' do
        min = weird_game.min
        expect(min).to eq weird_game.max
      end
